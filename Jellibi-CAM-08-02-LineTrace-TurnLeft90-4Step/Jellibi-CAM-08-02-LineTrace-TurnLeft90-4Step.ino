...
int Power = 80;  // 기본 주행(라인트레이싱) 속력 (약간 느리게)
...
void  loop()
{
  ...
  { // 양쪽 IR센서 모두 검정색을 감지한 경우, (ex) 검정 교차선
    ...
    while( analogRead(pinLT1) < LT_MAX_WHITE ) // 현재 정지선 벗어나기
    {
      TurnLeft( TURN_POWER ); // 좌회전 시작
      delay( 1 );
    }
    delay( 15 );
    
    while( analogRead(pinLT1) > LT_MIN_BLACK ) // 반대쪽 길 도착까지
    {
      TurnLeft( TURN_POWER ); // 본격 좌회전 진행
      delay( 1 );
    }
    delay( 10 );
    ...
    Forward( TURN_POWER ); // 좌회전 이후의 직진 시작
